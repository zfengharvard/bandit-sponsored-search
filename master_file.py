...
def regret_winexp(bidder, T,num_repetitions, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values, bids):
    winexp_regr = []
    for rep in range(0, num_repetitions):
        print ("Repetition rep %d for winexp"%rep)
        winexp_regr.append(main_winexp(bidder,rep, T, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values,bids))
    ...
    print (winexp_expected_regr/T)
    return (winexp_expected_regr/T)


def regret_exp3(bidder, T,num_repetitions, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values,bids):
    exp3_regr = [] 
    for rep in range(0, num_repetitions):
        print ("Repetition rep %d for exp3"%rep)
        exp3_regr.append(main_exp3(bidder,rep, T, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values,bids))
        # After each repetition, re-initialize the weights and the probabilities of the arm
        #for i in range(0, num_bidders):
        #    bidder[i].pi         = [bidder[i].eps for j in range(0,bidder[i].bid_space)]
        #    bidder[i].weights    = [1 for j in range(0,bidder[i].bid_space)]
