############ 
# Gets initialized from the auction_parameters.py file.
# Calls all other modules.
# Gets back the estimated regret (after 10 repetitions of each round are ran).
# Plots Regret curves.
############ 
...
epsilon = 0.001
bidder = [Bidder(i, epsilon, T, outcome_space, num_repetitions) for i in range(0,num_adaptive)]
cpy1 = deepcopy(bids)
cpy2 = deepcopy(bids)
cp3  = deepcopy(bids)
...
(winexp, winexp_regrets) = regret_winexp(bidder, T, num_repetitions, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values, cpy1, threshold,noise_cpy1,num_adaptive)
...
(exp3, exp3_regrets) = regret_exp3(bidder,T,num_repetitions, num_bidders, num_slots, outcome_space, rank_scores, ctr, reserve, values, cpy2,threshold,noise_cpy2,num_adaptive)
...
plt.savefig('winexp_adversaries.png')
